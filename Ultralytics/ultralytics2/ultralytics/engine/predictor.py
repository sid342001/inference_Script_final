    ...
    def preprocess(self, im: Union[torch.Tensor, List[np.ndarray]]) -> torch.Tensor:
        ...
        Args:
            im (torch.Tensor | List[np.ndarray]): 
                - Tensor: (N, C, H, W)
                - List of np.ndarray: [(H, W, C) x N], supports RGB or multi-band (e.g., TIFF 4-band)

        Returns:
            torch.Tensor: Preprocessed image tensor of shape (N, C, H, W).
        """
        not_tensor = not isinstance(im, torch.Tensor)
        ...
        if not_tensor:
            ...
            for arr in im:
                if arr.ndim != 3:
                    raise ValueError(f"Invalid image shape {arr.shape}, expected (H, W, C)")

                # --- Handle TIFF (4-band or higher) ---
                if arr.shape[-1] >= 4:
                    # take first 3 or 4 channels depending on model input
                    arr = arr[..., :4]  # keep 4 bands
                elif arr.shape[-1] == 3:
                    # normal RGB/BGR
                    arr = arr[..., ::-1]  # convert BGR->RGB if needed
            ...
            im = np.stack(imgs)  # BHWC
            im = im.transpose((0, 3, 1, 2))  # → BCHW
