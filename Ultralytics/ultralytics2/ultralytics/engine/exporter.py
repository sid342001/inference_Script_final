...
class Exporter:
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):
        ...
        if self.args.int8:
            ...
            def transform_fn(data_item) -> np.ndarray:
                """Quantization transform function. Supports uint8 and uint16 images."""
                data_item: torch.Tensor = data_item["img"] if isinstance(data_item, dict) else data_item

                # Check dtype and normalize accordingly
                if data_item.dtype == torch.uint8:
                    im = data_item.numpy().astype(np.float32) / 255.0
                elif data_item.dtype == torch.uint16:
                    im = data_item.numpy().astype(np.float32) / 65535.0
                else:
                    raise ValueError(f"Unsupported image dtype {data_item.dtype}. Expected uint8 or uint16.")
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):
        ...
        bias = [0.0, 0.0, 0.0]
        # scale = 1 / 255
        # Detect input bit depth and set normalization scale
        if self.im.dtype == torch.uint8:
            scale = 1 / 255.0
        elif self.im.dtype == torch.uint16:
            scale = 1 / 65535.0
        else:
            raise ValueError(f"Unsupported image dtype {self.im.dtype}, expected uint8 or uint16.")
