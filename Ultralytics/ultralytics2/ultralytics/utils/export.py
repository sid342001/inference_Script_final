    ...
    if int8:
        ...
        class EngineCalibrator(trt.IInt8Calibrator):
            ...
            def get_batch(self, names) -> Optional[List[int]]:
                """Get the next batch to use for calibration, as a list of device memory pointers."""
                try:
                    im0s = next(self.data_iter)["img"].float()

                    # Normalization for 8-bit and 16-bit
                    if im0s.dtype == torch.uint8:
                        im0s = im0s / 255.0
                    elif im0s.dtype == torch.uint16:
                        im0s = im0s / 65535.0
                    # else: already float in [0,1] or [0,255], assume caller handled it
